...
explicit statuses, and source evidence became exportable. The effect on historical
numbers remains unknown until the original snapshot is rerun.
...
**What is still unfinished?** The real-data run, source-panel checks, candidate
classifications and reconciliation of historical counts. Tests establish program
behavior on fixtures; they do not complete those research tasks.
...
3. When the dataset is available, follow one records.csv row back to
